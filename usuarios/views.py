...
def cadastro(request):
    if request.method == "GET":
        ...
        if users.exists():
            messages.add_message(request, constants.ERROR, "Já existe um usúario com este nome")
            return redirect('/usuarios/cadastro')
...
def login_view(request):
    if request.method == "GET":
        ...
        messages.add_messages(request, constants.ERROR, 'Usuário ou senha inválidos')
        return redirect ('/usuarios/login')
